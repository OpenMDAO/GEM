       ...
       double *fmin)                    /* (out) objective function after step */
{
  ...
  *fmin = 0;
  ...
  if (temp == NULL) {status = GEM_ALLOC; goto cleanup;}
...
int
gem_conjGrad(int    (*func)(int, double[], void *, double *, /*@null@*/ double[]),
                                       /* (in)  name of objective function */
             void   *blind,            /* (in)  blind pointer to structure */
             int    n,                 /* (in)  number of design variables */
             double x[],               /* (in)  initial set of design variables */
                                       /* (out) optimized design variables */
             ...
             double *fopt)             /* (out) optimized objective function */
{
    ...
    *fopt = 0;
    ...
    if (g    == NULL) {status = GEM_ALLOC; goto cleanup;}

    h    = (double*) malloc(n*sizeof(double));
    if (h    == NULL) {status = GEM_ALLOC; goto cleanup;}

    grad = (double*) malloc(n*sizeof(double));
    if (grad == NULL) {status = GEM_ALLOC; goto cleanup;}
