...
static int gem_spline(int natural, int nu, double *r, double *aux, double *t)
{
   ...
   */
    ...
    if (nu <= 1) {
        printf("gem_spline: cannot interpolate a spline with %d points\n", nu);
        return -1;
...
static int gem_fillCoeff1D(int nrank, int ntx, double *fit, double *coeff, 
                           double *r)
{
  ...
  for (i = 0; i < nrank; i++) {
    ...
    if (gem_spline(per, nt, r, aux, t)) return 1;
    for (j = 0; j < nt; j++) {
...
static void gem_eval1D(int nrank, int nt, double *coeff, double t, double *sv,
...
static void gem_invEval1D(int nrank, int ntx, double *coeff, double *sv, 
                          double *t)
{
  ...
  for (k = 0, step = 0.5; k < 20; k++, step *= 0.5) {
    ...
    if (cs < tmin || cs > tmax) {
      printf(" gem_invEval1D Info: cs = %10.5e %10.5e %10.5e\n",
             cs, tmin, tmax);
      *t = cs;
      return;
    }
...
static void gem_eval2D(int nrank, int nu, int nv, double *coeff, 
...
static int gem_newton2D(int nrank, int nu, int nv, double *coeff, double *sv, 
                        double *uv, double *tmp)
{
  ...
  while ((uv[0] >= 0.0) && (uv[0] <= (double)(nu-1)) &&
    ...
    gem_eval2D(nrank, nu, nv, coeff, uv, &tmp[0], &tmp[nrank], &tmp[2*nrank], 
...
static void gem_invEval2D(int nrank, int nux, int nvx, double *coeff, 
...
again:
    ...
    for (j = 0, jcv = kcv-4.0*stepv, jk = -1; j < 9; j++, jcv += stepv) {
      for (i = 0, jcu = kcu-4.0*stepu, ik = -1; i < 9; i++, jcu += stepu) {
        ...
        if ((jcu >= 0.0) && (jcu <= (double) (nu-1)) &&
          ...
          uv[0] = jcu;
          uv[1] = jcv;
          gem_eval2D(nrank, nu, nv, coeff, uv, &tmp[0], 
    ...
    uv[0] = cu;
    uv[1] = cv;
    if (gem_newton2D(nrank, nu, nv, coeff, sv, uv, tmp) == 0) {
                         ...
                         gemAprx1D *interp)
{
  ...
  if (npts <= 1) return GEM_BADVALUE;
  ts = (double *) gem_allocate(npts*sizeof(double));
  if (ts == NULL) return GEM_ALLOC;

  if (npts == 2) {
    nt  = npts;
    fit = (double *) gem_allocate(npts*nrank*sizeof(double));
    if (fit == NULL) {
      gem_free(ts);
      return GEM_ALLOC;
    }
    if (tx != NULL) {
      ntm  = npts;
      tfit = (double *) gem_allocate(npts*sizeof(double));
      if (tfit == NULL) {
        gem_free(fit);
        gem_free(ts);
        return GEM_ALLOC;
  ...
  if (tx != NULL) {
    ...
    for (i = 2; i < npts; i++) {
      sq = tx[i] - tx[i-1];
      if (sq*maxerr <= 0.0) {
        printf(" gem_Interp1DFit: Ts are not Monotonic!\n");
        gem_free(ts);
        return GEM_NOTPARAMBND;
      }
      maxerr = sq;
    }
  }
  ...
  if (stat != GEM_SUCCESS) {
    printf(" gem_Interp1DFit: prm_NormalizeU = %d!\n", stat);
    gem_free(ts);
    return GEM_NOTFOUND;
  }
  nt   = npts;
...
#ifdef DEBUG
  printf(" gem_Interp1DFit: prm_BestCfit Vals errors = %d %d  %lf %lf\n",
         stat, nt, rmserr, maxerr);
#endif
  if ((stat != GEM_SUCCESS) && (stat != PRM_TOLERANCEUNMET)) {
    printf(" gem_Interp1DFit: npts = %d, prm_BestCfit = %d\n", npts, stat);
    if (fit != NULL) gem_free(fit);
    gem_free(ts);
    return GEM_NOTFOUND;
  }
  if (fit == NULL) {
    printf(" gem_Interp1DFit: prm_BestCfit returns NULL\n");
    gem_free(ts);
    return GEM_NULLOBJ;
  }
  if (tx != NULL) {
...
#ifdef DEBUG
    printf(" gem_Interp1DFit: prm_BestCfit Ts   errors = %d %d  %lf %lf\n",
           stat, ntm, rmserr, maxerr);
#endif
    if ((stat != GEM_SUCCESS) && (stat != PRM_TOLERANCEUNMET)) {
      printf(" gem_Interp1DFit: Ts npts = %d, prm_BestCfit = %d", npts, stat);
      if (tfit != NULL) gem_free(tfit);
      gem_free(fit);
      gem_free(ts);
      return GEM_NOTFOUND;
    }
    if (tfit == NULL) {
      printf(" gem_Interp1DFit: prm_BestCfit returns NULL for Ts");
      gem_free(fit);
      gem_free(ts);
      return GEM_NOTFOUND;
    }
...
fill:
  ...
  r = (double *) gem_allocate(6*i*sizeof(double));
  if (r == NULL) {
    if (tfit != NULL) gem_free(tfit);
    gem_free(fit);
    gem_free(ts);
  ...
  coeff = (double *) gem_allocate(nrank*2*nt*sizeof(double));
  if (coeff == NULL) {
    gem_free(r);
    if (tfit != NULL) gem_free(tfit);
    gem_free(fit);
  ...
  if (tfit != NULL) {
    tmap = (double *) gem_allocate(2*ntm*sizeof(double));
    if (tmap == NULL) {
      gem_free(tmap);
      gem_free(r);
      gem_free(coeff);
  ...
  if (gem_fillCoeff1D(nrank, ntx, fit, coeff, r) == 1) {
    gem_free(tmap);
    gem_free(r);
    gem_free(coeff);
    if (tfit != NULL) gem_free(tfit);
  ...
  if ((tfit != NULL) && (tmap != NULL))
    if (gem_fillCoeff1D(1, ntm, tfit, tmap, r) == 1) {
      gem_free(tmap);
      gem_free(r);
      gem_free(coeff);
      gem_free(tfit);
...
/*  printf(" GEM Info: Unconnected Side %d %d = %d\n",
                    ...
                    int ntris, int *tris, double tol, gemAprx2D *interp)
{
  ...
  if ((npts == 0) || (ntris == 0)) return GEM_NULLVALUE;
  
  uvs   = (prmUV *)  gem_allocate( npts*sizeof(prmUV));
  vtris = (prmTri *) gem_allocate(ntris*sizeof(prmTri));
  if ((uvs == NULL) || (vtris == NULL)) {
    if (vtris != NULL) gem_free(vtris);
    if (uvs   != NULL) gem_free(uvs);
    return GEM_ALLOC;
  }
  for (i = 0; i < npts; i++) {
    uvs[i].u = uvx[2*i  ];
  ...
  stat = gem_triFill(npts, ntris, tris, vtris);
  if (stat != GEM_SUCCESS) {
    printf(" gem_Interp2DFit: gem_triFill = %d!\n", stat);
    gem_free(vtris);
    gem_free(uvs);
    return GEM_NULLOBJ;
  }
  ...
  if (nneg == ntris) {
    ...
    printf(" gem_Interp2DFit: Input has %d Negative (UV) tris (of %d)!\n",
           nneg, ntris);
    gem_free(vtris);
    gem_free(uvs);
    return GEM_NULLOBJ;
  ...
  gem_free(vtris);
  if ((stat != GEM_SUCCESS) && (stat != PRM_NOTCONVERGED)) {
    printf(" gem_Interp2DFit: prm_SmoothUV = %d!\n", stat);
    gem_free(uvs);
    return GEM_NULLOBJ;
  }
//#ifdef DEBUG
  if (stat == PRM_NOTCONVERGED)
    printf(" gem_Interp2DFit: prm_SmoothUV Not Converged!\n");
//#endif

  stat = prm_NormalizeUV(0.0, periodic, npts, uvs);
  if (stat != GEM_SUCCESS) {
    printf(" gem_Interp2DFit: prm_NormalizeUV = %d!\n", stat);
    gem_free(uvs);
    return GEM_NOTFOUND;
  }
  nu   = npts;
...
#ifdef DEBUG
  printf(" gem_Interp2DFit: prm_BestGrid Values errors = %d  %d %d  %lf %lf\n",
         stat, nu, nv, rmserr, maxerr);
#endif
  if ((stat != GEM_SUCCESS) && (stat != PRM_TOLERANCEUNMET)) {
    printf(" gem_Interp2DFit: prm_BestGrid = %d!\n", stat);
    if (fit != NULL) gem_free(fit);
    gem_free(uvs);
    return GEM_NOTFOUND;
  }
  if (fit == NULL) {
    printf(" gem_Interp2DFit: prm_BestGrid returns NULL!\n");
    gem_free(uvs);
    return GEM_NULLOBJ;
  }
  num  = 1.5*npts;
...
#ifdef DEBUG
  printf(" gem_Interp2DFit: prm_BestGrid UVs    errors = %d  %d %d  %lf %lf\n",
         stat, num, nvm, rmserr, maxerr);
#endif
  if ((stat != GEM_SUCCESS) && (stat != PRM_TOLERANCEUNMET)) {
    printf(" gem_Interp2DFit: prm_BestGrid UV = %d!\n", stat);
    if (uvfit != NULL) gem_free(uvfit);
    gem_free(fit);
    gem_free(uvs);
    return GEM_NOTFOUND;
  }
  if (uvfit == NULL) {
    printf(" gem_Interp2DFit: prm_BestGrid UV returns NULL!\n");
    gem_free(fit);
    gem_free(uvs);
    return GEM_NULLOBJ;
  }
...
/*	printf(" %d,%d: %lf %lf\n", j, i, uvfit[2*k], uvfit[2*k+1]);  */
  ...
  coeff = (double *) gem_allocate(nrank*4*nu*nv*sizeof(double));
  if (coeff == NULL) {
    gem_free(uvfit);
    gem_free(fit);
    gem_free(uvs);
  ...
  uvmap = (double *) gem_allocate(2*4*num*nvm*sizeof(double));
  if (uvmap == NULL) {
    gem_free(coeff);
    gem_free(uvfit);
    gem_free(fit);
  ...
  r = (double *) gem_allocate(6*maxsize*sizeof(double));
  if (r == NULL) {
    gem_free(uvmap);
    gem_free(coeff);
    gem_free(uvfit);
  ...
  if (gem_fillCoeff2D(nrank, nux, nvx, fit, coeff, r) == 1) {
    gem_free(r);
    gem_free(uvmap);
    gem_free(coeff);
    gem_free(uvfit);
  ...
  if (gem_fillCoeff2D(2, num, nvm, uvfit, uvmap, r) == 1) {
    gem_free(r);
    gem_free(uvmap);
    gem_free(coeff);
    gem_free(uvfit);
                      ...
                      /*@null@*/ double *dvv)
{
  ...
  store = (double *) gem_allocate(nrank*5*sizeof(double));
  if (store == NULL) return GEM_ALLOC;
