 ...
 ********************************************************************************
 *                                                                              *
 * prmUV.c -- This module creates and manages parameterizations                 *
 *                                                                              *
...
#define MALLOC(PTR,TYPE,SIZE) \
        ...
        PTR = (TYPE *) gem_allocate((SIZE) * sizeof(TYPE)); \
        if (PTR == NULL) {\
...
#define RALLOC(PTR,TYPE,SIZE) \
        ...
        realloc_temp = gem_reallocate(PTR, (SIZE) * sizeof(TYPE)); \
        if (PTR == NULL) {\
                        ...
                        prmUV      uv[])     /* (out)  array of  Parameters */
{
     ...
     */
    MALLOC(amat, double, SQR(nvrt));

    for (ij = 0; ij < nvrt*nvrt; ij++) {
        amat[ij] = 0;
    }
    ...
    for (itri = 0; itri < ntri; itri++) {
        ...
        if (newTri[itri].neigh[1] > 0) {
            ...
            amat[iv0+iv2*nvrt] = (tan(ang0/2) + tan(ang3/2)) / sqrt(d20sq);
        }
        ...
        if (newTri[itri].neigh[2] > 0) {
            ...
            amat[iv1+iv0*nvrt] = (tan(ang1/2) + tan(ang4/2)) / sqrt(d01sq);
        }
        ...
        if (newTri[itri].neigh[0] > 0) {
            ...
            amat[iv2+iv1*nvrt] = (tan(ang2/2) + tan(ang5/2)) / sqrt(d12sq);
        }
    }
    ...
    for (i = 0; i < nvrt; i++) {
        for (j = 0; j < nvrt; j++) {
            if (i!=j && fabs(amat[i+j*nvrt]) > EPS20) {
                k++;
                asmf[k] = amat[i+j*nvrt];
                ismf[k] = j;
            }
        }
          ...
          prmTri   *newTri[])                /* (out)  new array  of Tri */
{
    ...
    int         itri, oldNtri, oldIvrt, iv0, iv1, iv2;
    fillArea    fast;
    ...
    *newNtri = ntri;
    ...
    for (ilup = 0; ilup < nlup; ilup++) {
        ...
        for (ipass = 0; ipass <= 1; ipass++) {
            status = gem_fillArea(ncont, cont, uvcont, tempTri, &nfig8, ipass,
                                  &fast);
            DPRINT1("      gem_fillArea: status=%d", status);
            if (status > 0) break;
        }
        if (fast.segs  != NULL) gem_free(fast.segs);
        if (fast.pts   != NULL) gem_free(fast.pts);
        if (fast.front != NULL) gem_free(fast.front);
          ...
          double   rhs[])                    /* (in)   right-hand side */
{
    ...
    double      err, bnrm, rmin, rmax, check;
    double      ak,        akden;
...
prm_CreateUV(int      UVtype,                /* (in)   type of transformation (see status) */
  ...
  /*@null@*/ int      pindx[],               /* (in)   array  of point indices */
             ...
             int      *ppnts[])              /* (out)  pointer to indices of periodic points
                                                       NOTE: user must gem_free after use */
{
    int         status = PRM_NOGLOBALUV;     /* (out)  return status */
                                             /*        PRM_NOGLOBALUV */
                                             /*        PRM_OK_ONEFACE */
 ...
 ********************************************************************************
 *                                                                              *
 * prm_SmoothUV -- reparameterize based upon are-lengths and angles             *
 *                                                                              *
 ********************************************************************************
 */
extern int
...
//$$$    }
    ...
    MALLOC(itab,  int,        nvrt );
    MALLOC(ttab,  double,     nvrt );
    MALLOC(stab,  double,     nvrt );
    MALLOC(utab,  double,     nvrt );
    MALLOC(vtab,  double,     nvrt );
    MALLOC(amat,  double, SQR(nvrt));
    ...
    if (type == 2 || type == 3) {
        ...
        for (itri = 0; itri < newNtri; itri++) {
            ...
            if (newTri[itri].neigh[1] > 0) {
                ...
                amat[(iv0)+nvrt*(iv2)] = (tan(ang0/2) + tan(ang3/2)) / sqrt(d20sq);
            }
            ...
            if (newTri[itri].neigh[2] > 0) {
                ...
                amat[(iv1)+nvrt*(iv0)] = (tan(ang1/2) + tan(ang4/2)) / sqrt(d01sq);
            }
            ...
            if (newTri[itri].neigh[0] > 0) {
                ...
                amat[(iv2)+nvrt*(iv1)] = (tan(ang2/2) + tan(ang5/2)) / sqrt(d12sq);
            }
        }
        ...
        for (i = 0; i < nvrt; i++) {
            for (j = 0; j < nvrt; j++) {
                if (i != j && fabs(amat[(i)+nvrt*(j)]) > EPS20) {
                    k++;
                    asmf[k] = amat[(i)+nvrt*(j)];
                    ismf[k] = j;
                }
            }
