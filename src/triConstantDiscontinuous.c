 ...
 *
 *             Example Triangle Piecewise Constant Discontinuous
 *
...
/*
 *
 * This simple (constant) example of a triangle-based discretization has its 
 * data stored at the triangle centroid (i.e. cell-based data). There is a 
 * single type throughout.
 *
...
*/
  ...
  quilt->types[0].ndata = 1;
  quilt->types[0].ntri  = 1;
  quilt->types[0].nmat  = 1;
  ...
  quilt->types[0].dst   = (double *) malloc(2*sizeof(double));
  if (quilt->types[0].dst == NULL) {
    gemFreeQuilt(quilt);
    return GEM_ALLOC;
  }
  quilt->types[0].dst[0] = 1.0/3.0;
  quilt->types[0].dst[1] = 1.0/3.0;

  quilt->types[0].matst = (double *) malloc(2*sizeof(double));
  if (quilt->types[0].matst == NULL) {
    gemFreeQuilt(quilt);
    return GEM_ALLOC;
  }
  quilt->types[0].matst[0] = 1.0/3.0;
  quilt->types[0].matst[1] = 1.0/3.0;
  ...
  quilt->verts = (gemFaceUV *) malloc(ntris*sizeof(gemFaceUV));
  if (quilt->verts == NULL) {
    gemFreeQuilt(quilt);
    return GEM_ALLOC;
  }
  storage = (int *) malloc(ntris*4*sizeof(int));
  if (storage == NULL) {
    gemFreeQuilt(quilt);
    return GEM_ALLOC;
  }
  ...
  for (ntris = npts = i = j = 0; j < quilt->nbface; j++) {
    ...
    for (k = 0; k < drep->TReps[ibrep].Faces[iface].ntris; k++, i++) {
      ...
      uv[0] = (drep->TReps[ibrep].Faces[iface].uvs[2*i0  ] +
               drep->TReps[ibrep].Faces[iface].uvs[2*i1  ] +
               drep->TReps[ibrep].Faces[iface].uvs[2*i2  ])/3.0;
      uv[1] = (drep->TReps[ibrep].Faces[iface].uvs[2*i0+1] +
               drep->TReps[ibrep].Faces[iface].uvs[2*i1+1] +
               drep->TReps[ibrep].Faces[iface].uvs[2*i2+1])/3.0;
      quilt->verts[i].owner    = j+1;
      quilt->verts[i].uv[0]    = uv[0];
      quilt->verts[i].uv[1]    = uv[1];
      quilt->elems[i].tIndex   = 1;
      quilt->elems[i].owner    = j+1;
      quilt->elems[i].gIndices = &storage[4*i  ];
      quilt->elems[i].dIndices = &storage[4*i+3];
      storage[4*i  ] = i0 + 1 + npts;
      storage[4*i+1] = i1 + 1 + npts;
      storage[4*i+2] = i2 + 1 + npts;
      storage[4*i+3] = i+1;
    }
    npts  += drep->TReps[ibrep].Faces[iface].npts;
    ntris += drep->TReps[ibrep].Faces[iface].ntris;
               ...
               double   result[])     /* (out) integrated result - (rank) */
{
  ...
  if (geomFlag == 1) {
    ...
    j = quilt->elems[eIndex-1].dIndices[0] - 1;
    for (i = 0; i < rank; i++)
      result[i] = data[rank*j+i] * area;
  }
...
/*    result[i] = area*(data[rank*in[0]+i] + data[rank*in[1]+i] +
    ...
    for (i = 0; i < rank; i++) {
/*    result[i] = data[rank*j+i] * area;  */
      dat_bar[rank*j+i] += area*res_bar[i];
    }
  }
